...
export interface RefreshResult { accessToken: string; tokenType: string }
export const authApi = api.injectEndpoints({ endpoints: build => ({
  ...
  refresh: build.mutation<RefreshResult, void>({ query: () => ({ url: '/auth/refresh', method: 'POST' }) }),
...
export const { useLoginMutation, useSignupMutation, useVerifyEmailMutation, useRefreshMutation, useMeQuery, useLogoutMutation } = authApi;
